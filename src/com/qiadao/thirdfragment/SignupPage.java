package com.qiadao.thirdfragment;
...
public class SignupPage extends FakeActivity implements OnClickListener, Callback {
	...
	public void onCreate() {                                       
		...
		activity.findViewById(R.id.authorize).setOnClickListener(new OnClickListener() {
			
			@Override
			public void onClick(View v) {
				if(PersonalFragment.PersonalFragment!=null){
					PersonalFragment.PersonalFragment.finish();	
				}
			...
			public void run() {
				try {
			        ...
			        try {  
			        	...
			        	userIcon.compress(Bitmap.CompressFormat.JPEG, 100, b);// 把数据写入文�? 
...
//		});
	...
	private Bitmap compressImageFromFile(String srcPath) {
		...
		newOpts.inPreferredConfig = Config.ARGB_8888;//该模式是默认�?可不�?		newOpts.inPurgeable = true;// 同时设置才会有效
